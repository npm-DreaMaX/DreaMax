...
- 首页增加三个 WebGL 场景：训练阶段切换连续金属曲面、Agent 机械光圈、项目螺旋带 / 互锁环 / 流动线束；Scholar 页面使用同系列光圈。支持拖动、方向键、暂停、展开、重置，以及金属 / 磨砂材质、打光方向和运动速度调节，按可见区域加载，最高 30fps / DPR 1.5，屏外和后台停止，缺少 WebGL 时显示 SVG。
